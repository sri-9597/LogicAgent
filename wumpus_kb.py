...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for i in range(len(tvec)):
        prefix = '~' if tvec[i] is False else ''
        percept_clauses.append(prefix + proposition_bases_perceptual_fluents[i] + str(t))
    axiom_str = ' & '.join(percept_clauses)
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    axioms = [pit_str(x, y)]
    if x - 1 >= xmin:
        axioms.append(pit_str(x - 1, y))
    if x + 1 <= xmax:
        axioms.append(pit_str(x + 1, y))
    if y - 1 >= ymin:
        axioms.append(pit_str(x, y - 1))
    if y + 1 <= ymax:
        axioms.append(pit_str(x, y + 1))
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    if x - 1 >= xmin:
        axioms.append(wumpus_str(x - 1, y))
    if x + 1 <= xmax:
        axioms.append(wumpus_str(x + 1, y))
    if y - 1 >= ymin:
        axioms.append(wumpus_str(x, y - 1))
    if y + 1 <= ymax:
        axioms.append(wumpus_str(x, y + 1))
    axiom_str += '(' + ' | '.join(axioms) + ')'
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for i in range(length):
        for j in range(i + 1, length):
            x1, y1 = int(i / xmax) + 1, int(i % xmax) + 1
            x2, y2 = int(j / ymax) + 1, int(j % ymax) + 1
            temp = '(' + '~' + wumpus_str(x1, y1) + ' | ' + '~' + wumpus_str(x2, y2) + ')'
            axioms.append(temp)
    axiom_str = ' & '.join(axioms)
    return axiom_str
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t=0):
    ...
    """
    "*** YOUR CODE HERE ***"
    axioms = [state_loc_str(xi, yi, t)]
    for x in range(xmax + 1):
        for y in range(ymax + 1):
            if y != yi or x != xi:
                axioms.append('~' + state_loc_str(x, y, t))
    axiom_str = ' & '.join(axioms)
    return axiom_str
...
def axiom_generator_only_one_heading(heading='north', t=0):
    ...
    """
    "*** YOUR CODE HERE ***"
    heading_map = {'north': state_heading_north_str(t), 'east': state_heading_east_str(t),
                   'south': state_heading_south_str(t), 'west': state_heading_west_str(t)}
    axioms = [heading_map[heading]]
    for head in heading_map:
        if head != heading:
            axioms.append('~' + heading_map[head])
    axiom_str = ' & '.join(axioms)
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    axioms = ['(' + state_loc_str(x, y, t) + ' & ' + '(' + '~' + action_forward_str(t) + ' | ' + percept_bump_str(t+1) + ')' + ')']
    if x-1 >= xmin:
        axioms.append('(' + state_loc_str(x-1, y, t) + ' & ' + '(' + state_heading_east_str(t) + ' & ' + action_forward_str(t) + ')' + ')')
    if x+1 <= xmax:
        axioms.append('(' + state_loc_str(x+1, y, t) + ' & ' + '(' + state_heading_west_str(t) + ' & ' + action_forward_str(t) + ')' + ')')
    if y-1 >= ymin:
        axioms.append('(' + state_loc_str(x, y-1, t) + ' & ' + '(' + state_heading_north_str(t) + ' & ' + action_forward_str(t) + ')' + ')')
    if y+1 <= ymax:
        axioms.append('(' + state_loc_str(x, y+1, t) + ' & ' + '(' + state_heading_south_str(t) + ' & ' + action_forward_str(t) + ')' + ')')
...
def axiom_generator_heading_north_ssa(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_north_str(t+1) + ' <=> ' + '((' + state_heading_north_str(t) + ' & ' + '~' + '(' + action_turn_right_str(t) + ' | ' + action_turn_left_str(t) + ')' + ')' + ' | ' + '(' + state_heading_west_str(t) + ' & ' + action_turn_right_str(t) + ')' + ' | ' + '(' + state_heading_east_str(t) + ' & ' + action_turn_left_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_east_str(t+1) + ' <=> ' + '((' + state_heading_east_str(t) + ' & ' + '~' + '(' + action_turn_right_str(t) + ' | ' + action_turn_left_str(t) + ')' + ')' + ' | ' + '(' + state_heading_north_str(t) + ' & ' + action_turn_right_str(t) + ')' + ' | ' + '(' + state_heading_south_str(t) + ' & ' + action_turn_left_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_south_str(t+1) + ' <=> ' + '((' + state_heading_south_str(t) + ' & ' + '~' + '(' + action_turn_right_str(t) + ' | ' + action_turn_left_str(t) + ')' + ')' + ' | ' + '(' + state_heading_east_str(t) + ' & ' + action_turn_right_str(t) + ')' + ' | ' + '(' + state_heading_west_str(t) + ' & ' + action_turn_left_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_west_str(t+1) + ' <=> ' + '((' + state_heading_west_str(t) + ' & ' + '~' + '(' + action_turn_right_str(t) + ' | ' + action_turn_left_str(t) + ')' + ')' + ' | ' + '(' + state_heading_south_str(t) + ' & ' + action_turn_right_str(t) + ')' + ' | ' + '(' + state_heading_north_str(t) + ' & ' + action_turn_left_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_only_north(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_north_str(t) + ' <=> ' + '(' + '~' + '(' + state_heading_east_str(t) + ' | ' + state_heading_west_str(t) + ' | ' + state_heading_south_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_only_east(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_east_str(t) + ' <=> ' + '(' + '~' + '(' + state_heading_north_str(t) + ' | ' + state_heading_west_str(t) + ' | ' + state_heading_south_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_only_south(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_south_str(t) + ' <=> ' + '(' + '~' + '(' + state_heading_east_str(t) + ' | ' + state_heading_west_str(t) + ' | ' + state_heading_north_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_only_west(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_west_str(t) + ' <=> ' + '(' + '~' + '(' + state_heading_east_str(t) + ' | ' + state_heading_north_str(t) + ' | ' + state_heading_south_str(t) + '))'
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    for i in proposition_bases_actions:
        ...
        for j in proposition_bases_actions:
            if j != i:
                temp_axioms.append(j + str(t))
        temp = '(' + i + str(t) + ' <=> ' + '~' + '(' + ' | '.join(temp_axioms) + ')' + ')'
        axioms.append(temp)
    axiom_str = ' & '.join(axioms)
    return axiom_str
